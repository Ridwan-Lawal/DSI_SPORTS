...
export default function NewsByCategories() {
  return (
    <div className="space-y-3">
      ...
      {/*  the link {`/news?category=${category}`} */}
      {/* news by category */}
      <div className="no-scrollbar flex items-center gap-4 overflow-x-scroll px-4 py-4">
        {Array.from({ length: 9 })?.map((_, id) => (
          <div
            key={id}
            className="w-[170px] shrink-0 cursor-pointer space-y-2 rounded-md border border-neutral-100 bg-white px-4 py-3 shadow-md shadow-neutral-100 transition-all hover:scale-105 hover:border-2 hover:border-neutral-800"
          >
